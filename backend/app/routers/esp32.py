    ...
    try:
        ...
        if not device_info["success"]:
            ...
            try:
                ...
                device_info["message"] = "Device not responsive or info endpoints not available"
    ...
    try:
        ...
        for octet in octets:
            if not (0 <= int(octet) <= 255):
                raise HTTPException(status_code=400, detail="Invalid IP address: octets must be between 0-255")
    ...
    try:
        async with httpx.AsyncClient(timeout=3.0) as client:
            ...
            return {
                ...
                "response_time_ms": round(response_time, 2),
                "online": response.status_code == 200,
                "message": "Device is online" if response.status_code == 200 else "Device is offline"
            }
        ...
        return {
            ...
            "online": False,
            "error": "Connection refused",
            "message": "Connection refused - device may be offline"
        }
    except httpx.TimeoutException:
        return {
            "success": False,
...
async def scan_esp32_devices():
    ...
    try:
            ...
            for ip in common_ips:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        try:
                            info_response = await client.get(f"http://{ip}/info", timeout=1.0)
        ...
        return {
            ...
            "discovered_devices": discovered_devices,
            "total_found": len(discovered_devices),
            "message": f"Found {len(discovered_devices)} ESP32 devices"
        }
