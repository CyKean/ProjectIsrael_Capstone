...
async def get_latest_sensor_data(db = Depends(get_database)):
    ...
    try:
        ...
        # Get ESP32-1 data (NPK and pH)
        device_doc = await sensor_readings.find_one({"_id": "esp32-1"})
        if device_doc and device_doc.get("readings") and device_doc["readings"]:
            # Get the most recent reading (last in array since they're sorted by timestamp descending)
            latest_reading = device_doc["readings"][0]  # First element is most recent due to sorting
            result["esp32-1"] = serialize_document(latest_reading)
        
        # Get ESP32-2 data (Environmental)
        device_doc = await sensor_readings.find_one({"_id": "esp32-2"})
        if device_doc and device_doc.get("readings") and device_doc["readings"]:
            latest_reading = device_doc["readings"][0]  # First element is most recent
            result["esp32-2"] = serialize_document(latest_reading)
        
        # Get ESP32-3 data (Water Level) - get latest water level
        data = await water_level_readings.find_one(
            sort=[("timestamp._seconds", -1)]  # Sort by Firestore timestamp seconds
        )
        if data:
            result["esp32-3"] = serialize_document(data)
...
async def get_all_sensor_data(db = Depends(get_database)):
    ...
    try:
        ...
        device_doc = await sensor_readings.find_one({"_id": "esp32-1"})
        if device_doc and device_doc.get("readings"):
            # Get the most recent readings (already sorted by timestamp descending)
            recent_readings = device_doc["readings"][:50]  # First 50 are most recent
            ...
            for reading in recent_readings:
                ...
                result["npkData"].append({
                    ...
                    "potassium": float(reading.get("potassium", 0)),
                    "soilPh": float(reading.get("soilPh", 7.0)),
                    "device_id": reading.get("device_id", "ESP32-NPKPH"),
                    "timestamp": reading_timestamp or datetime.now()
                })
        ...
        if device_doc and device_doc.get("readings"):
            ...
            recent_readings = device_doc["readings"][:50]
            ...
            for reading in recent_readings:
                ...
                result["envData"].append({
                    ...
                    "humidity": float(reading.get("humidity", 0)),
                    "soilMoisture": float(reading.get("soilMoisture", 0)),
                    "device_id": reading.get("device_id", "ESP32-ENV"),
                    "timestamp": reading_timestamp or datetime.now()
                })
        
        # Get ESP32-3 data (Water Level) - get the 50 most recent readings
        cursor = water_level_readings.find().sort("timestamp._seconds", -1).limit(50)
        water_data = await cursor.to_list(length=50)
        for item in water_data:
            if item:
                item_timestamp = item.get("timestamp")
                ...
                result["waterData"].append({
                    "waterLevel": float(item.get("waterLevel", 0)),
                    "device_id": item.get("device_id", "ESP32-WATER"),
                    "timestamp": item_timestamp or datetime.now()
                })
...
async def get_latest_water_level(db = Depends(get_database)):
    ...
    try:
        ...
        data = await water_level_readings.find_one(
            sort=[("timestamp._seconds", -1)]  # Sort by Firestore timestamp
        )
        ...
        # Return default if no data found
        return {"waterLevel": 0, "timestamp": datetime.now().isoformat(), "device_id": "ESP32-WATER"}
...
async def get_motor_history(
    ...
    try:
        ...
        # Process timestamps first - convert all to timezone-naive datetime objects
        processed_data = []
        for item in history_data:
            item_timestamp = item.get("timestamp")
            ...
            if isinstance(item_timestamp, str):
                try:
                    ...
                    processed_item["timestamp"] = parsed_timestamp.replace(tzinfo=None)
                except Exception as e:
            ...
            # Handle regular datetime (make timezone-naive)
                ...
                processed_item["timestamp"] = item_timestamp.replace(tzinfo=None)
            else:
        ...
        if week_only:
            ...
            # Sunday is day 6 in Python (0=Monday, 6=Sunday)
            days_since_sunday = (now.weekday() + 1) % 7
            start_of_week = now - timedelta(days=days_since_sunday)
            start_of_week = start_of_week.replace(hour=0, minute=0, second=0, microsecond=0)
            
            print(f"Start of week (Sunday): {start_of_week}")
            ...
            for item in history_data:
                ...
                if item_timestamp and item_timestamp >= start_of_week:
                    filtered_history.append(item)
                    print(f"Included: {item_timestamp} - status: {item.get('status')}")
            
            history_data = filtered_history
        
        # Sort by timestamp (newest first) - all timestamps are now timezone-naive
        history_data.sort(key=lambda x: x.get("timestamp", datetime.now()), reverse=True)
...
async def create_sensor_reading(
    ...
    try:
        ...
        # Add timestamp if not provided (in Firestore format)
        if "timestamp" not in reading:
            reading["timestamp"] = {
                "_seconds": int(datetime.now().timestamp()),
                "_nanoseconds": 0
            }
        
        # Ensure device_id is present (not deviceId)
        device_id = reading.get("device_id") or reading.get("deviceId")
        if not device_id:
            raise HTTPException(status_code=400, detail="device_id is required")
        
        # Insert into appropriate collection based on device type
        if device_id.startswith("ESP32-WATER"):
            water_reading = {
                "waterLevel": reading.get("waterLevel", 0),
                "timestamp": reading["timestamp"],
                "device_id": device_id
            ...
            # Determine sensor type based on device_id
            sensor_type = "esp32-2" if "ENV" in device_id else "esp32-1"
            
            # For sensor devices, add to the readings array
            device_doc = await sensor_readings.find_one({"_id": sensor_type})
            
            # Prepare the reading with proper format
            sensor_reading = {
                "_id": str(ObjectId()),
                "device_id": device_id,
                "timestamp": reading["timestamp"]
            ...
            if sensor_type == "esp32-1":
                sensor_reading.update({
                    ...
                    "soilPh": reading.get("soilPh", 7.0)
                })
            else:
                sensor_reading.update({
                    "temperature": reading.get("temperature", 0),
                    "humidity": reading.get("humidity", 0),
                    "soilMoisture": reading.get("soilMoisture", 0)
                })
