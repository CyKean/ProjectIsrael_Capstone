...
def get_latest_timestamp(readings):
    """Get the latest reading based on timestamp"""
    if not readings:
        return None
    ...
    for reading in readings:
        ...
        if isinstance(timestamp, dict) and '_seconds' in timestamp:
            current_timestamp = convert_firestore_timestamp(timestamp)
        elif isinstance(timestamp, datetime):
...
async def get_latest_sensor_data():
    try:
        ...
        # Get NPK data from esp32-1
        esp32_1 = await db["sensor_readings"].find_one({"_id": "esp32-1"})
        latest_npk = None
        if esp32_1 and "readings" in esp32_1 and len(esp32_1["readings"]) > 0:
            latest_npk = get_latest_timestamp(esp32_1["readings"])
        
        # Get environmental data from esp32-2
        esp32_2 = await db["sensor_readings"].find_one({"_id": "esp32-2"})
        latest_env = None
        if esp32_2 and "readings" in esp32_2 and len(esp32_2["readings"]) > 0:
            latest_env = get_latest_timestamp(esp32_2["readings"])
...
async def get_filtered_recommendations(
    ...
    try:
        ...
        # Get paginated results - sort by timestamp descending
        skip = (page - 1) * limit
        cursor = collection.find(query).sort("timestamp", -1).skip(skip).limit(limit)
        ...
        async for doc in cursor:
            ...
            if timestamp:
                if isinstance(timestamp, dict) and '_seconds' in timestamp:
                    timestamp_dt = convert_firestore_timestamp(timestamp)
                    formatted_date = timestamp_dt.strftime("%b %d, %Y, %I:%M %p")
                elif isinstance(timestamp, datetime):
                    formatted_date = timestamp.strftime("%b %d, %Y, %I:%M %p")
                else:
                    formatted_date = str(timestamp)
...
async def save_crop_recommendation(data: dict = Body(...)):
    ...
    try:
        ...
        # Prepare the document data with Firestore timestamp
        doc_data = {
            "recommendedCrop": data.get("recommendedCrop", ""),
            "successRate": data.get("successRate", 0.0),
            "soilCompatibility": data.get("soilCompatibility", 0.0),
...
async def get_saved_recommendations():
    try:
        ...
        # Get recommendations sorted by timestamp descending (using Firestore timestamp format)
        cursor = collection.find().sort("timestamp._seconds", -1)
        recommendations = []
        ...
        async for doc in cursor:
            ...
            if timestamp:
                try:
                    ...
                    if isinstance(timestamp, dict) and "_seconds" in timestamp:
                        timestamp_dt = datetime.fromtimestamp(timestamp["_seconds"])
                        formatted_date = timestamp_dt.strftime("%b %d, %Y at %I:%M:%S %p")
                        timestamp_iso = timestamp_dt.isoformat()
                    # Handle other timestamp formats
                    elif isinstance(timestamp, datetime):
                        formatted_date = timestamp.strftime("%b %d, %Y at %I:%M:%S %p")
                        timestamp_iso = timestamp.isoformat()
                    else:
                        formatted_date = str(timestamp)
                        timestamp_iso = str(timestamp)
