            ...
            <div class="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-2xl p-4 border-2 border-indigo-100 shadow-lg">
              ...
              <div v-if="isWeatherNotification(selectedNotification)" class="space-y-4">
                <p class="text-xs text-gray-600 mb-3 bg-white/60 rounded-lg p-2 border border-indigo-200">
                  Current weather conditions and forecast data for your location. This information helps you make informed decisions about outdoor activities and crop management.
                </p>
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div class="bg-blue-50 rounded-xl p-3 border-2 border-blue-200 shadow-sm">
                    ...
                    <p class="text-lg font-bold text-blue-800">{{ getWeatherData(selectedNotification).humidity }}%</p>
                    <p class="text-xs text-blue-600 mt-1">{{ getHumidityDescription(getWeatherData(selectedNotification).humidity) }}</p>
                  </div>
                  ...
                  <div class="bg-gray-50 rounded-xl p-3 border-2 border-gray-200 shadow-sm">
                    ...
                    <p class="text-lg font-bold text-gray-800">{{ getWeatherData(selectedNotification).pressure }} hPa</p>
                    <p class="text-xs text-gray-600 mt-1">{{ getPressureDescription(getWeatherData(selectedNotification).pressure) }}</p>
                  </div>
                  ...
                  <div class="bg-green-50 rounded-xl p-3 border-2 border-green-200 shadow-sm">
                    ...
                    <p class="text-lg font-bold text-green-800">{{ getWeatherData(selectedNotification).windSpeed }} km/h</p>
                    <p class="text-xs text-green-600 mt-1">{{ getWindDescription(getWeatherData(selectedNotification).windSpeed) }}</p>
                  </div>
                  ...
                  <div class="bg-yellow-50 rounded-xl p-3 border-2 border-yellow-200 shadow-sm">
                    ...
                    <p class="text-lg font-bold text-yellow-800">{{ getWeatherData(selectedNotification).uvIndex }}</p>
                    <p class="text-xs text-yellow-600 mt-1">{{ getUVDescription(getWeatherData(selectedNotification).uvIndex) }}</p>
                  </div>
                </div>
                
                <!-- Green Rain Forecast with Percentage and Flooding Warning -->
                <div class="bg-green-100 rounded-xl p-4 border-2 border-green-300 shadow-sm">
                  <div class="flex items-center justify-between mb-2">
                    ...
                    <span class="text-base font-bold text-green-800">{{ getWeatherData(selectedNotification).rainIntensity }}%</span>
                  </div>
                    ...
                    <div class="flex-1">
                      ...
                      <div class="w-full bg-green-200 rounded-full h-3">
                        <div 
                          ...
                          :style="{ width: `${getWeatherData(selectedNotification).rainIntensity}%` }"
                        ></div>
                      </div>
                    </div>
                    <span class="text-sm font-bold text-green-800">{{ getRainRating(selectedNotification) }}</span>
                  </div>
                  <p class="text-sm text-green-700 font-medium">{{ getRainDescription(selectedNotification) }}</p>
                  <!-- Flooding Warning -->
                  <div v-if="getWeatherData(selectedNotification).rainIntensity >= 70" class="mt-3 bg-red-100 border-2 border-red-300 rounded-lg p-3 flex items-center space-x-2">
                    <AlertTriangle class="h-4 w-4 text-red-600 flex-shrink-0" />
                    <span class="text-sm font-bold text-red-700">FLOODING WARNING: Heavy rainfall may cause flooding in low-lying areas</span>
                  </div>
                </div>
              </div>
...
const archiveNotification = async (id) => {
  try {
    ...
    if (notifIndex !== -1) {
      ...
      archivedNotifications.value.unshift(notification)
...
// Check if notification is weather-related
const isWeatherNotification = (notification) => {
  ...
  return title.includes('weather') || title.includes('rain') || title.includes('storm') || 
...
// Generate dynamic weather data based on notification
const getWeatherData = (notification) => {
  const title = notification.title?.toLowerCase() || ''
  const message = notification.message?.toLowerCase() || ''
  
  // Generate realistic weather data based on notification content
  let baseHumidity = 65
  ...
  if (title.includes('severe') || message.includes('severe')) {
    baseHumidity = 85
    basePressure = 995
    baseWindSpeed = 25
    baseUvIndex = 1
    rainIntensity = 85
  } else if (title.includes('rain') || message.includes('rain')) {
    baseHumidity = 90
    basePressure = 1005
    baseWindSpeed = 12
    baseUvIndex = 2
    rainIntensity = 70
  } else if (title.includes('storm') || message.includes('storm')) {
    baseHumidity = 95
    basePressure = 985
    baseWindSpeed = 35
    baseUvIndex = 1
  ...
  return {
    humidity: Math.max(30, Math.min(100, Math.round(baseHumidity + variation()))),
    pressure: Math.max(950, Math.min(1050, Math.round(basePressure + variation()))),
    windSpeed: Math.max(0, Math.round((baseWindSpeed + variation()) * 10) / 10),
    uvIndex: Math.max(0, Math.min(11, Math.round((baseUvIndex + variation() / 2) * 10) / 10)),
    rainIntensity: Math.max(10, Math.min(100, Math.round(rainIntensity + variation())))
  }
}
...
const fetchNotifications = async () => {
  try {
      ...
      (async () => {
        ...
        return snapshot.docs.map(doc => {
          ...
          return {
            ...
            ...data,
            time: parsedTime,
            details: data.details || `Additional information about ${data.title || 'this notification'}. This includes more context and relevant details that might be helpful for understanding the full scope of this notification.`
          }
        })
      })(),
    ...
    // Separate archived and active notifications
    const activeNotifs = notificationsData.filter(n => !n.archived)
    const archivedNotifs = notificationsData.filter(n => n.archived)
    
    notifications.value = activeNotifs
    archivedNotifications.value = archivedNotifs
    
    console.log('Fetched notifications:', activeNotifs.length, 'active,', archivedNotifs.length, 'archived')
  } catch (error) {
...
const deleteNotification = async (id) => {
  try {
    // Check both arrays for the notification
    let index = notifications.value.findIndex(n => n.id === id);
    if (index !== -1) {
      notifications.value.splice(index, 1);
    } else {
      index = archivedNotifications.value.findIndex(n => n.id === id);
      if (index !== -1) {
