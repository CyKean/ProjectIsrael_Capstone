<!-- <template>
  <div class="guide-container">
    <button class="guide-button" @click="startTour">
...
const startTour = () => {
  introJs().setOptions({
    steps: [
      { element: '[data-intro-motor]', intro: 'This is your motor control. Tap to toggle the irrigation pump.', position: 'right' },
      { element: '[data-intro-status]', intro: 'Current motor status will show as ACTIVE or INACTIVE.', position: 'right' },
      { element: '[data-intro-schedule-button]', intro: 'Click here to add a new watering schedule.', position: 'bottom' },
      { element: '[data-intro-next-watering]', intro: 'See the next scheduled watering here.', position: 'bottom' },
      { element: '[data-intro-history-button]', intro: 'Click here to review your watering history.', position: 'bottom' }
    ],
...
const startTour = () => {
  ...
  scrollToElement('[data-intro-motor]')
  scrollToElement('[data-intro-status]')
  scrollToElement('[data-intro-schedule-button]')
  scrollToElement('[data-intro-next-watering]')
  scrollToElement('[data-intro-history-button]')
  ...
  setTimeout(() => {
    ...
    requestAnimationFrame(() => {
      ...
      tour.setOptions({
        steps: [
          { 
            element: '[data-intro-motor]', 
            intro: 'This is your motor control. Tap to toggle the irrigation pump.', 
            position: 'right' 
          },
          { 
            element: '[data-intro-status]', 
            intro: 'Current motor status will show as ACTIVE or INACTIVE.', 
            position: 'right' 
          },
          { 
            element: '[data-intro-schedule-button]', 
            intro: 'Click here to add a new watering schedule.', 
            position: 'bottom' 
          },
          { 
            element: '[data-intro-next-watering]', 
            intro: 'See the next scheduled watering here.', 
            position: 'bottom' 
          },
          { 
            element: '[data-intro-history-button]', 
            intro: 'Click here to review your watering history.', 
            position: 'bottom' 
          }
        ],
        showStepNumbers: false,
      ...
      // When motor step is active, make it smaller
      tour.onbeforechange((targetElement) => {
        // Reset all elements first
        document.querySelectorAll('[data-intro-motor], [data-intro-status], [data-intro-schedule-button], [data-intro-next-watering], [data-intro-history-button]').forEach(el => {
          el.style.height = ''
          el.style.transform = ''
          el.style.transition = ''
          el.style.zIndex = ''
        ...
        // If motor container is the target, make it smaller
        if (targetElement && targetElement.hasAttribute('data-intro-motor')) {
          targetElement.style.height = '60%' // Reduce height to 70%
          targetElement.style.transform = 'scale(0.95)' // Slightly smaller scale
          targetElement.style.transition = 'all 0.3s ease'
          targetElement.style.zIndex = '10010' // Move to upper layer
        }
...
:global(.introjs-overlay) {
  background-color: rgba(75, 72, 72, 0.85) !important;
}
...
/* Special styling for motor container when highlighted - SMALLER HEIGHT */
:global([data-intro-motor].introjs-showElement) {
  height: 60% !important;
